...
- **miniscript-csharp.dll** — the managed .NET assembly (the interpreter itself)
- **miniscript-csharp.runtimeconfig.json** — runtime configuration required by the .NET host
...
## Usage
    ...
    dotnet miniscript-csharp.dll [script.ms]

If no script file is given, the interpreter starts in interactive (REPL) mode.

## Why use this build?

The native binaries (miniscript-mac, miniscript-linux, miniscript-win.exe)
