...
namespace MiniScript {
	...
	// VM state
		...
		Int32 VM::CallStackDepth() {
			return callStackTop;
		}

		Value VM::GetStackValue(Int32 index) {
			if (index < 0 || index >= stack.Count()) return make_null();
			return stack[index];
		}

		Value VM::GetStackName(Int32 index) {
			if (index < 0 || index >= names.Count()) return make_null();
			return names[index];
		}

		CallInfo VM::GetCallStackFrame(Int32 index) {
			if (index < 0 || index >= callStackTop) return CallInfo(0, 0, -1);
			return callStack[index];
		}

		String VM::GetFunctionName(Int32 funcIndex) {
		...
		void VM::InitVM(Int32 stackSlots, Int32 callSlots) {
			stack = List<Value>();
			names = List<Value>();
		...
		void VM::RegisterFunction(FuncDef funcDef) {
			functions.Add(funcDef);
		}

		void VM::Reset(List<FuncDef> allFunctions) {
		...
		void VM::RaiseRuntimeError(String message) {
			RuntimeError = message;
			IsRunning = false;
		}
		
		bool VM::ReportRuntimeError() {
		...
		Int32 VM::ProcessArguments(Int32 argCount, Int32 startPC, Int32 callerBase, Int32 calleeBase, FuncDef callee, List<UInt32>& code) {
		...
		// Note: Parameters start at r1 (r0 is reserved for return value)
		void VM::SetupCallFrame(Int32 argCount, Int32 calleeBase, FuncDef callee) {
			Int32 paramCount = callee.ParamNames.Count();
		...
		Value VM::Execute(FuncDef entry) {
			return Execute(entry, 0);
		}

		Value VM::Execute(FuncDef entry, UInt32 maxCycles) {
		...
		Value VM::Run(UInt32 maxCycles) {
			...
			FuncDef& curFunc = CurrentFunction; // should be: FuncDef& curFunc = CurrentFunction;
		...
		void VM::EnsureFrame(Int32 baseIndex, UInt16 neededRegs) {
		...
		Value VM::LookupVariable(Value varName) {
		...
		const Value VM::FuncNamePrint = make_string("print");
		const Value VM::FuncNameInput = make_string("input");
		const Value VM::FuncNameVal = make_string("val");
		const Value VM::FuncNameRemove = make_string("remove");
		
		void VM::DoIntrinsic(Value funcName, Int32 baseReg) {
