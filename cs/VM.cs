...
public class VM {
	...
	*** END H_ONLY ***/
	...
	public String FindShortName(Value v) {
		...
		if (_globals != null) {
			...
			for (Int32 i = 0; i < slotCount; i++) {
				...
				if (!slotName.RefEquals(v)) return slotName.ToString(null);
			}
		}
	...
	public static void MarkRoots(object user_data) {
		...
		if (vm._globals != null) GCManager.Mark(vm._globals.AsMap());
	...
	public Value RunFunction(Value funcRef, List<Value> args) {
		...
		Int32 safeTop = BaseIndex + (CurrentFunction != null ? CurrentFunction.MaxRegs : 0);
		...
		if (callStackTop >= callStack.Count) callStack.Add(new CallInfo(0, 0, null));
		callStack[callStackTop] = new CallInfo(savedPC, savedBase, savedFunc, -1, funcRef.OuterVars());
		callStackTop++;
...
//*** END CS_ONLY ***
	...
	*** END CPP_ONLY ***/
	...
	public Value GetGlobalsVarMap() {
		if (_globals == null) return Value.Null;
	...
	private Value LookupVariable(Value varName) {
		...
		if (_globals != null) {
			if (_globals.AsMap().TryGet(varName, out result)) {
				return result;
			}
		}
