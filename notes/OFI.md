...
- **`VM.cs` `SetupCallFrame` ~963–979**: the entry loop clears both `stack[]` and `names[]` over the callee's `MaxRegs`.  Now that `RETURN` erases a frame's names as it pops (so a callee's leftover names can no longer be picked up by the enclosing frame's `VarMapBacking`), the names half of that entry loop is largely redundant: a frame base is always past the caller's in-use registers, so nothing the caller has named can live there.  Dropping it would halve the stores in the hottest part of a call, but only once the remaining exits are shown to maintain the invariant — error unwinding does not go through `RETURN`, and `Reset`/`RunFunction` restore `callStackTop` directly.
