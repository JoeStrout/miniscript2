...
- `isFrozen(x)` will return true if x is frozen, false if not.
...
And, using any list or map as a map key actually uses a `frozenCopy` on assignment (automatically).  And any list or map literal used as a default parameter value is also automatically frozen.
