...
public class Assembler {
	...
	public UInt32 AddLine(String line, Int32 lineNumber) {
		...
		if (mnemonic == "NOOP") {
			...
			Byte obj = ParseRegister(parts[2]);
			Byte key = ParseRegister(parts[3]);
			instruction = BytecodeUtil.INS_ABC(Opcode.IDXGET_rA_rB_rC, dest, obj, key);
	...
	// An 8-bit operand the VM reads back as signed (the iC field of the
	// comparison opcodes).  Accepts -128..127 and stores the two's-complement
	// byte, so a negative immediate survives the round trip.
	private Byte ParseSByte(String num) {
		return (Byte)(SByte)ParseIntRange(num, SByte.MinValue, SByte.MaxValue, "SByte");
	}
